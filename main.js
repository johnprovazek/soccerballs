...
// Set the viewer mode ('debug', 'stitch', or 'default').
...
let soccerBallIndex = 0; // Index to track the current soccer ball being displayed in the viewer.
var geometry = null; // Geometry containing the soccer ball vertices.
var mesh = null; // Mesh for the soccer ball.
var materials = []; // List of materials for each soccer ball design.
let textures = {}; // Key-pair mapping of textures on the soccer balls.
let images = []; // List of keys corresponding to images that have been loaded and updated in the textures and materials.
let stitchImages = [null,null]; // Hexagon and pentagon stitch images.
let shapes = [ // Defining soccer ball hexagon and pentagon shape details.
  {
    'type': 'hexagon',
    'count': 20,
    'triangles': 54,
...
// Camera variables.
let distance = 5; // Start distance of the camera.
let diameter = 2; // Diameter of the soccer ball.
let marginPercentage = 10; // Margin percentage between the soccer ball and window border to allow for the name and buttons.
let paddingPercentage = 2; // Extra padding between the soccer ball and window border.
let fullHeight = diameter / (1 - ((marginPercentage + paddingPercentage) / 50)); // Full height of the soccer ball and window border that is used to position the soccer ball in the viewer.
...
window.addEventListener('load', function(event) {
  loadJSON('./public/data/soccerballs.json', function(text){
    // Parsing the JSON file containing position vertices, uv mapping, and texture information into an object.
    soccerBalls = JSON.parse(text);
    // Removing the debug soccer ball if not in debug mode.
...
document.getElementById('soccer-ball-button-prev').addEventListener('click', function() {
  changeSoccerBall(-1);
});

// Handles pressing the 'next' soccer ball button.
document.getElementById('soccer-ball-button-next').addEventListener('click', function() {
  changeSoccerBall(1);
});
...
function loadDefaultTexture(){
  const transparentTexture = textureLoader.load('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAAtJREFUGFdjYAACAAAFAAGq1chRAAAAAElFTkSuQmCC');
    ...
    for (let j = 0; j < soccerBalls['designs'][index][type].length; j++){
      ...
      if(!(images.includes(key)) && !(missingImages.some(m => m.key === key))){
        missingImages.push({
          key: key,
          type: type,
          path: './public/textures/panels/' + type + '/' + panelName + '.png'
        });
      }
    }
  }
  ...
  if (missingImages.length){
    ...
    if((mode === 'stitch' || mode === 'debug') && stitchImages.some(s => s === null)){
      ...
      for (let i = 0; i < shapes.length; i++) {
        ...
        stitchImage.src = './public/textures/templates/' + shapes[i]['type'] + '/stitch.png';
      }
    }
    else{
      loadTextures();
        ...
        for (let j = 0; j < numSides; j++) {
          ...
          if(mode === 'debug'){
            ...
            for (let k = 0; k < numSides; k++) {
              ...
              context.strokeText(rotationNum, canvas.width * rotationCoordinates[0], canvas.height * rotationCoordinates[1]);
              context.fillText(rotationNum, canvas.width * rotationCoordinates[0], canvas.height * rotationCoordinates[1]);
            }
          }
    ...
    for(let i = 0; i < shapes.length; i++){
      ...
      for (let j = 0; j < soccerBalls['designs'][index][type].length; j++) {
        ...
        if(rotation < 0 || rotation > shapes[i]['sides'] - 1){
          alert('Invalid rotation value (' + rotation + ') used. For ' + shapes[i]['name'] + ' shapes use a number from 0 to ' + String(shapes[i]['sides'] - 1) + '. Setting rotation value to 0.');
          rotation = 0;
        }
        material.push(
          new THREE.MeshBasicMaterial({
...
function loadDebugging() {
  if(mode === 'debug'){
    ...
    const dotTexture = loader.load('./public/images/dot.png');
